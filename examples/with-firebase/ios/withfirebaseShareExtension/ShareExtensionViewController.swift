...
class KeyboardExtensionViewController: UIViewController {
  ...
  private func initializeReactNativeBridgeIfNeeded() {
    if KeyboardExtensionViewController.sharedBridge == nil {
      let jsCodeLocation = self.jsCodeLocation()
      KeyboardExtensionViewController.sharedBridge = RCTBridge(bundleURL: jsCodeLocation, moduleProvider: nil, launchOptions: nil)
    }
  }
  
  private func loadReactNativeContent() {
    getShareData { [weak self] sharedData in
      guard let self = self, let bridge = KeyboardExtensionViewController.sharedBridge else { return }
  ...
  private func cleanupAfterClose() {
    ...
    rootView = nil
    KeyboardExtensionViewController.sharedBridge?.invalidate()
    KeyboardExtensionViewController.sharedBridge = nil
