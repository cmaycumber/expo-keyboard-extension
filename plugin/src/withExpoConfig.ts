...
import {
  getAppGroups,
  getKeyboardExtensionBundleIdentifier,
...
export const withExpoConfig: ConfigPlugin = (config) => {
  ...
  const extensionName = getKeyboardExtensionName(config);
  const extensionBundleIdentifier = getKeyboardExtensionBundleIdentifier(config);
